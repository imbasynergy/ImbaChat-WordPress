...
class Ping {
   ...
   */
  public function __construct($host, $ttl = 255, $timeout = 20) {
    if (!isset($host)) {
  ...
  private function pingExec() {
    ...
    if (!empty($output[1])) {
      ...
      $response =preg_match_all("/[\.0-9]+/", $output[1], $matches);
      
      // If there's a result and it's greater than 0, return the latency.
      if ($response > 0 && isset($matches[0][1])) {
        $latency = round($matches[0][1], 4);
