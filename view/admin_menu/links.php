<h1 class="IMCH_title"><?php _("ImbaChat Links") ?></h1>
...
<form id="link_imba_form" class="IMCH_form" method="post">
    ...
    <div>
        <h3><?php _("Please, enter your e-mail, registered on the ImbaChat website. It will be checked and the links will be shown below.") ?></h3>
        <div class="IMCH_form__field">
            <label class="IMCH_form__label" for="email"><?php _("Your email address") ?></label>
            <input id="email" name="im_email" class="IMCH_form__input" value="<?= get_option( 'admin_email' ) ?>">
        </div>
        <div class="IMCH_form__field">
            <button class="IMCH_form__button" type="submit" id="submit_imba"><?php _("Submit") ?></button>
        </div>
    </div>
</form>
<div id="links_block">
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#links_block").html(
                        ...
                        '        <button class="IMCH_form__button" onclick="go_to(\''+imbachat_link+'\')"><?php _("Admin panel") ?></button>\n' +
                        '    </div>\n' +
                        '    <div class="IMCH_form__field inline" style="display: inline-block">\n' +
                        '        <button class="IMCH_form__button" onclick="go_to(\''+dashboard_link+'\')"><?php _("Dashboard panel") ?></button>\n' +
                        '    </div>'
                    )
                }
            })
